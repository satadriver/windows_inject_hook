...
DWORD processNameToId(LPCSTR lpszProcessName)
{
    HANDLE hSnapshot = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);
    PROCESSENTRY32 pe;
    pe.dwSize = sizeof(PROCESSENTRY32);
    if (!Process32First(hSnapshot, &pe)) {
        MessageBoxA(NULL, "The frist entry of the process list has not been copyied to the buffer", "Notice", MB_ICONINFORMATION | MB_OK);
        return 0;
    }
    wchar_t procname[1024];
    MultiByteToWideChar(CP_ACP, 0, lpszProcessName, -1, procname, sizeof(procname));
    do {
...
int main_old(int argc,char * argv[])
{
    ...
    if (lstrcmpiA(argv[1], ("-n")) == 0) {
        const char* szExeName = argv[2];
        dwProcessId = processNameToId(szExeName);
    }
    ...
    if (dwProcessId == 0) {
        MessageBoxA(NULL, "The target process have not been found !", "Notice", MB_ICONINFORMATION | MB_OK);
        return -1;
    }

    HANDLE hTargetProcess = OpenProcess(PROCESS_ALL_ACCESS, FALSE, dwProcessId);
    if (!hTargetProcess) {
        MessageBoxA(NULL, "Open target process failed !", "Notice", MB_ICONINFORMATION | MB_OK);
        return 0;
    }
    BOOL bWow64 = IsWow64(hTargetProcess);
    if (bWow64 || !Is64BitOS())
    {
        ...
        if (!pRemoteThread) {
            MessageBoxA(NULL, "Alloc memory in target process failed !", "notice", MB_ICONINFORMATION | MB_OK);
            return 0;
        }
        if (!WriteProcessMemory(hTargetProcess, pRemoteThread, &threadProc, dwThreadSize, 0)) {
            MessageBoxA(NULL, "Write data to target process failed !", "Notice", MB_ICONINFORMATION | MB_OK);
            return 0;
        }
            ...
            if (pCode != NULL)
            {
                ...
                if (bStatus) {
                    ...
                    if (hThread != NULL)
                    {
                        ...
                        if (idx != 0) {
                            MessageBoxA(NULL, "WaitForSingleObject", "Notice", MB_ICONINFORMATION | MB_OK);
                        }
                        ec = 0;
                        ...
                        MessageBoxA(NULL, "CreateRemoteThread", "Notice", MB_ICONINFORMATION | MB_OK);
                    }
                }
                if (idx == 0) {
                ...
                MessageBoxA(NULL, "VirtualFreeEx()", "Notice", MB_ICONINFORMATION | MB_OK);
            }
            CloseHandle(hProc);
        }
        else {
            MessageBoxA(NULL, "OpenProcess", "Notice", MB_ICONINFORMATION | MB_OK);
        }
